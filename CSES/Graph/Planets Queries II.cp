...
int main(){
  ...
  /*FOR(i, 1, n){
    cout << i << ":";
    FOR(j, 0, cont[i].size()) cout << " " << cont[i][j];
    cout << " p: " << p[i] << " pos: " << pos[i] << " cy: " << cy[i] << "\n";
  }*/
  
  FOR(i, 0, qr){
    cin >> x >> y;
    k = 0;
    while(p[x] != p[y] and !cy[x]){
    ...
    if(p[x] == p[y]){
      if(cy[x]){
        if(pos[x] > pos[y]) cout << (k + pos[y] + (cont[p[x]].size() - pos[x])) << "\n";
        else cout << (k + pos[y] - pos[x]) << "\n";
      }else{
        if(pos[x] <= pos[y]) cout << (k + (pos[y] - pos[x])) << "\n";
        else cout << "-1\n";
      }
    }else cout << "-1\n";
  }
